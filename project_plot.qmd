...
ggplot(long_data, aes(x = Date, y = `Data Series`, fill = Percent_Change)) +
  ...
  # more space between y-axis elements
  labs(title = "Percent Change In Consumer Price Index (CPI) Over Corresponding Period Of Previous Year, Monthly",
       x = NULL, y = NULL, fill = "Percent Change") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(plot.title = element_text(size = 15), 
        axis.text.x = element_text(size = 10), 
        axis.text.y = element_text(size = 10, margin = margin(0, 0, 0, 20)),  # Add margin to y-axis text
